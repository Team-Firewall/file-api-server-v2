...
struct UserData {
    ...
    number:Option<i8>,
    phone:String,
    parentsPhoneNum1:Option<String>,
    parentsPhoneNum2:Option<String>,
    account:String,
    password:String
}
    ...
    if let Some(Ok(r)) = excel.worksheet_range("Sheet1") {
        for row in r.rows() {
            result.push(UserData {
                role: row[0].get_float().unwrap() as i16,
                ...
                parentsPhoneNum1:Some(row[6].to_string()),
                parentsPhoneNum2:Some(row[7].to_string()),
                account:row[8].to_string(),
